...
def apply_sparsity(tile, target_sparsity_percent, seed=None, axis='row'):
    """
    Adjusts the sparsity of the tile to match the target percentage by zeroing out entire rows or columns.
    - tile: 2D numpy array
    - target_sparsity_percent: desired sparsity (0-100)
    - seed: for reproducibility
    - axis: 'row' for row-wise, 'col' for column-wise structured sparsity
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    # Calculate how many rows/columns to zero out
    if axis == 'row':
        elements_per_block = cols
        blocks = rows
    else:  # axis == 'col'
        elements_per_block = rows
        blocks = cols

    num_blocks_to_zero = int((target_sparsity_percent / 100) * total_elements / elements_per_block)

    # Zero out blocks (rows or columns)
    if axis == 'row':
        # Zero out the last num_blocks_to_zero rows
        tile[-num_blocks_to_zero:, :] = 0
        print(f"Structured sparsity: zeroed {num_blocks_to_zero} rows for {target_sparsity_percent}% sparsity.")
    else:
        # Zero out the last num_blocks_to_zero columns
        tile[:, -num_blocks_to_zero:] = 0
        print(f"Structured sparsity: zeroed {num_blocks_to_zero} columns for {target_sparsity_percent}% sparsity.")
...
def main():
    ...
    if inputMatrix_data is None or inputMatrix_weight is None:
        ...
        sparsity_percent = 12.5*5  # Set this from 0 to 100 as needed
