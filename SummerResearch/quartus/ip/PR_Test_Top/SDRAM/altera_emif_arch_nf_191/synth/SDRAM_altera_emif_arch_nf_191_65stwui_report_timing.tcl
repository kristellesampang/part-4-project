...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/SDRAM_altera_emif_arch_nf_191_65stwui_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
SDRAM_altera_emif_arch_nf_191_65stwui_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_SDRAM_altera_emif_arch_nf_191_65stwui_corename}"
   } else {
      set fbasename "${::GLOBAL_SDRAM_altera_emif_arch_nf_191_65stwui_corename}_${inst_id}"
   }
   ...
   SDRAM_altera_emif_arch_nf_191_65stwui_perform_read_capture_analysis $opcname $inst pins var summary

   #######################################
   # Write Analysis

   SDRAM_altera_emif_arch_nf_191_65stwui_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   SDRAM_altera_emif_arch_nf_191_65stwui_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   SDRAM_altera_emif_arch_nf_191_65stwui_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis

   SDRAM_altera_emif_arch_nf_191_65stwui_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core

   SDRAM_altera_emif_arch_nf_191_65stwui_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance

   set summary [lsort -command SDRAM_altera_emif_arch_nf_191_65stwui_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [SDRAM_altera_emif_arch_nf_191_65stwui_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_SDRAM_altera_emif_arch_nf_191_65stwui_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_SDRAM_altera_emif_arch_nf_191_65stwui_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ SDRAM_altera_emif_arch_nf_191_65stwui_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ SDRAM_altera_emif_arch_nf_191_65stwui_round_3dp $hold]
      }
