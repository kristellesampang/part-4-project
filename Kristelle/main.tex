...
\section{Introduction} \label{sec: intro}
    ...
    \begin{itemize}
        ...
        \item Validate and verify the system's behaviour through simulation to quantify the latency reduction and performance speed up of the proposed design compared to a baseline systolic array architecture.
...
\section{Design and Methodology} \label{sec: design_method}
    This section details the design and methodology presented in this project. It begins with the software design for data generation, pre-processing, tiling, and file generation. Following this, hardaware design of the Baseline Systolic Array architecture is presented, before detailing the proposed Optimised Systolic Array archiutecture that exploits structured sparsity. Finally, the section concludes with the hardware implementation on an FPGA platform.

    \subsection{Data Generation and Pre-processing} \label{sec: data_gen}    This covers the software side of the co-design, where it utilises AlexNet to generate and extract realistic weight and activation matrices, then pre-processes it into tiles suitable for hardware testing. The processed tiles are then saved as a MIF file and stored into the FPGA's ROMs for static testing. This section is crucial for ensuring hardware is tested with real-world data to validate its effectiveness in handling sparsity. Additionally, this design was implemented in a straight forward manner, meaning there were no major iterations or challenges faced.
...
\section{Verification and Results} \label{sec: verification}
    ...
    \newline
    The theoretical throughput (measured in Operations per Second, OPS) can be calculated on the average latency and clock speed. The following knowledge is re-iterated below:
    \begin{itemize}
        \item 1 MAC operation is equivalent to 2 operations (1 multiplication + 1 addition).
        \item AlexNet requires 710 million MAC operations, which translates to 1.42 billion operations.
        \item The FPGA operates at an Fmax of 289.27MHz.
    \end{itemize}
    Using this information, the throughput can be calculated as follows:
    \begin{equation}
        \text{Time to complete AlexNet} = \frac{\text{Total Cycles}}{Fmax} = \frac{710M MACs}{289.27MHz} = 2.45 \text{ seconds}
    \end{equation}
    % \newline
    \begin{equation}
    ...
    \end{equation}
    \begin{equation}
        \text{Baseline Systolic Array Throughput} =  \frac{1.42 \text{G MACs}}{2.45 \text{ seconds}} = 0.578 \text{GOPS}
    \end{equation}
    \begin{equation}
        \text{Optimised Systolic Array Throughput} = \frac{1.42 \text{G MACs}}{2.45 \text{ seconds} * \left(\frac{9.07}{23}\right)} = 1.47 \text{GOPS}
    \end{equation}
    \newline
    From this, the speedup can be derived as follows:
    \begin{equation}
        \text{Speedup} = \frac{\text{Optimised Throughput}}{\text{Baseline Throughput}} = \frac{1.47 \text{GOPS}}{0.578 \text{GOPS}} = 2.54x
    \end{equation}
