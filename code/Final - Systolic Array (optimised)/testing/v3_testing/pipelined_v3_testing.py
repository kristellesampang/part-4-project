...
# --- Constants ---
IMAGE_PATH = 'C:/Users/OEM/Documents/part-4-project/Final/Python/cat.jpg'
# IMAGE_PATH = 'C:/Users/OEM/Documents/part-4-project/Final/Python/hand_xray.jpg'
# IMAGE_PATH = 'C:/Users/OEM/Documents/part-4-project/Final/Python/patella_alta.jpg'
# MIF_OUTPUT_DIR = "C:/Users/OEM/Documents/part-4-project/Final/mif/pipeline_v2"
MIF_OUTPUT_DIR = "C:/Users/OEM/Documents/part-4-project/Final/testing/v2_alexnet/run_2"
# TEST_DATA_MIF_DIR = 'C:/Users/OEM/Documents/part-4-project/Final/testing/v2_alexnet/run_1/tile_1/activation_tile_1.mif'
# TEST_WEIGHT_MIF_DIR = 'C:/Users/OEM/Documents/part-4-project/Final/testing/v2_alexnet/run_1/tile_1/weight_tile_1.mif'
STRIPPED_DATA_MIF_DIR = 'C:/Users/OEM/Documents/part-4-project/Final/testing/v2_alexnet/run_2/tile_1/stripped_activation.mif'
STRIPPED_WEIGHT_MIF_DIR = 'C:/Users/OEM/Documents/part-4-project/Final/testing/v2_alexnet/run_2/tile_1/stripped_weight.mif'
LAYER_SIZE = 64
TILE_SIZE = 8
...
def sparsity_analysis(model, image_directory, convIndex, reluIndex):

    # analyse the images in the selected directory to collect the average sparsity and latency statistics

    original_sparsity_list = []
    compacted_sparsity_list = []
    all_m_values = []
    all_n_values = []
    all_k_values = []

    # files for all images in the directory
    ...
    # status print for starting analysis
    print(f"\nStarting sparsity analysis on {len(image_files)} images in directory: {image_directory}")
    ...
    for i, image_path in enumerate(image_files):
        ...
        # Extract weights and activations for the chosen conv and relu layers
        conv_weights_2d, conv_activations_2d = extract_conv_weights_and_activations(model, input_tensor, conv_idx=convIndex, relu_idx=reluIndex)

        # process each 8x8 tile
        for r in range(0, conv_weights_2d.shape[0], TILE_SIZE):
            for c in range(0, conv_weights_2d.shape[1], TILE_SIZE):
                # ensure that it dont go out of bounds
                if r + TILE_SIZE > conv_weights_2d.shape[0] or c + TILE_SIZE > conv_activations_2d.shape[1]:
                    continue

                w_tile = conv_weights_2d[r:r+TILE_SIZE, c:c+TILE_SIZE]
                #slice and transpose the activation tile
                a_tile = conv_activations_2d[c:c+TILE_SIZE, r:r+TILE_SIZE].T
         
                if w_tile.shape != (TILE_SIZE, TILE_SIZE) or a_tile.shape != (TILE_SIZE, TILE_SIZE):
                    continue

                # Original sparsity calculation (dense 8x8)
                original_latency = TILE_SIZE + TILE_SIZE + TILE_SIZE - 1
                original_sparsity_list.append(original_latency)

                # stripped latency calculation (after coordinated stripping)
                stripped_data, stripped_weight, m, k, n = coordinated_row_removal(a_tile, w_tile)

                # append the m n k values to the list
                ...
                if m > 0 and n > 0 and k > 0:
                    stripped_latency = m + n + k - 1
                    compacted_sparsity_list.append(stripped_latency)
                else:
                    compacted_sparsity_list.append(0)

    # Calculate average sparsity and latency
    avg_original_latency = np.mean(original_sparsity_list) if original_sparsity_list else 0
    avg_compacted_latency = np.mean(compacted_sparsity_list) if compacted_sparsity_list else 0
    latency_reduction = ((avg_original_latency - avg_compacted_latency) / avg_original_latency * 100) if avg_original_latency > 0 else 0

    # print("\n--- Sparsity Analysis Summary ---")
    # print(f"Total images processed: {len(image_files)}")
    # print(f"Average Original Latency (dense 8x8): {avg_original_latency:.2f} cycles")
    # print(f"Average Compacted Latency (after stripping): {avg_compacted_latency:.2f} cycles")
    # print(f"Latency Reduction: {latency_reduction:.2f}%")
    # print("\n" + "="*40 + "\n")

    # # Print average m, n, k values
    # print(f"Average m value: {np.mean(all_m_values):.2f}")
    # print(f"Average n value: {np.mean(all_n_values):.2f}")
    # print(f"Average k value: {np.mean(all_k_values):.2f}")
    # print("="*50)
    return {
        "original_latencies": original_sparsity_list,
        "stripped_latencies": compacted_sparsity_list,
        "all_m_values": all_m_values,
        "all_n_values": all_n_values,
        "all_k_values": all_k_values
    }

def plot_results(results_data):

    sns.set_style("whitegrid") # Makes the plots look nice
    
    # --- 1. Create the Bar Chart for Average Latency ---
    avg_original = np.mean(results_data["original_latencies"])
    avg_stripped = np.mean(results_data["stripped_latencies"])
    
    plt.figure(figsize=(8, 6))
    bars = plt.bar(
        x=['Baseline (Dense 8x8)', 'Our NPU (Compacted)'],
        height=[avg_original, avg_stripped],
        color=['#bc0a0a', '#305aaa'] # Red for baseline, Blue for ours
    )
    
    plt.ylabel('Average Clock Cycles per Tile')
    plt.title('Performance Comparison: Baseline vs. Optimized NPU')
    ...
    bar_chart_path = 'C:/Users/OEM/Documents/part-4-project/Final/Python/graph_files/latency_comparison_bar_chart.png'
    plt.savefig(bar_chart_path)
    print(f"\nBar chart saved to: {bar_chart_path}")
    plt.close() # Close the plot to free up memory

    # --- 2. Create the Histogram of Stripped Latencies ---
    plt.figure(figsize=(10, 6))
    sns.histplot(data=results_data["stripped_latencies"], bins=20, kde=True)
    
    plt.xlabel('Latency in Clock Cycles for a Single Tile')
    plt.ylabel('Number of Tiles (Frequency)')
    plt.title('Distribution of Optimized Tile Latencies')

    histogram_path = 'C:/Users/OEM/Documents/part-4-project/Final/Python/graph_files/latency_distribution_histogram.png'
...
def main():
    ...
    #profile ALEXNET model for total MACs
    print("\n--- ALEXNET MODEL PROFILING ---")
    unquantized_model = models.alexnet(weights=models.AlexNet_Weights.DEFAULT)
    input_tensor = torch.randn(1, 3, 224, 224)

    total_macs, total_params = profile(unquantized_model, inputs=(input_tensor,), verbose=False)

    print(f"Total MACs for AlexNet: {total_macs / 1e9:.2f} GMACS")
    
    # new data collection directory
    image_directory = 'C:/Users/OEM/Documents/part-4-project/Final/testing/sparsity_analysis_images'

    # analyse the first conv layer (conv0, relu1)
    #sparsity_analysis(model, image_directory, convIndex=0, reluIndex=1)
    results = sparsity_analysis(model, image_directory, convIndex=0, reluIndex=1)

        # Pass the collected data to the new plotting function
    if results:

        num_tiles = len(results["stripped_latencies"])
        print(f"\nNumber of tiles found: {num_tiles}")
        if num_tiles > 0:
            plot_results(results)
            
            # You can add the summary print statements here if you still want to see them
            avg_original = np.mean(results["original_latencies"])
            avg_stripped = np.mean(results["stripped_latencies"])
            avg_reduction = 100 * (1 - (avg_stripped / avg_original))
            print("\n--- Sparsity Analysis Summary ---")
            print(f"Average Baseline Latency: {avg_original:.2f} cycles")
            print(f"Average Optimized Latency: {avg_stripped:.2f} cycles")
            print(f"Latency Reduction: {avg_reduction:.2f}%")
        else:
            print("No tiles were processed; skipping plotting.")
